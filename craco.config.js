...
webpackConfig.devServer = (devServerConfig) => {
  // Strip webpack-dev-server v4 options injected by react-scripts 5 that are
  // invalid under the installed webpack-dev-server v5 (schema mismatch crash).
  const before = devServerConfig.onBeforeSetupMiddleware;
  const after = devServerConfig.onAfterSetupMiddleware;
  delete devServerConfig.onBeforeSetupMiddleware;
  delete devServerConfig.onAfterSetupMiddleware;

  // 'https' option was replaced by 'server' in webpack-dev-server v5.
  if ("https" in devServerConfig) {
    const https = devServerConfig.https;
    delete devServerConfig.https;
    if (https && !devServerConfig.server) {
